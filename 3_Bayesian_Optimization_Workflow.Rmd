...
In the rest of this section, the objective function is shown with $\overline{J}(\mathbf{u})$, cosnisten with the Equation \@ref(eq:npvopt). However, for simplicity, we drop the bar and writes the $\overline{J}(\mathbf{u})$ with $\mathbf{J}(\mathbf{u})$. Morever, $\mathbf{u}$ is the control decision, with dimention of $D$,  $\mathbf{u}=[u_1,\cdots,u_D]$. While, the capital letter, $\mathbf{U}$ is collection of $\mathbf{N}$ points of $\mathbf{u}$, defined as: $\mathbf{U}= [\mathbf{u_1},\cdots,\mathbf{u_N}]$.

The workflow of BO can bedivides into two steps:

- Step 1: Choose some initial design points $\mathcal{D}=\{{\mathbf{U},\mathbf{J(U)}}\}$ to build probabilistic model infered from $\mathcal{D}$
- Step 2: Deciding on next $\mathbf{u}^{next}$ and evalaute $\mathbf{J(u^{next})}$  based on probalistic model and $\mathcal{D}=\mathcal{D}\: \cup[\mathbf{u}^{next},\mathbf{J(u^{next})}]$

After the Step 2, we come back to the Step 1 with the new $\mathcal{D}$ and we itearte this process until we are out of computional budjet. We will explain these two steps n depth, but before we Gaussian Process is introduced, as it is needed as background for explaining the steps.
...
As it shown in the \@ref(tab:cov-tab), the Matern Covariance function with $\nu=\frac{5}{2}$ has two parametrs to be estimated, namely $\sigma^2_f$ and $\ell$. We call these two parameter as kernel parameters and can be shown as
...
The reason it is called the marginal likelihood, rather than just likelihood, is because we have marginalized out the latent Gaussian vector $\mathbf{J_U}$. The $log$ of marginal likelihood then can be writeen as:
...
### Step.2 Deciding on next $\mathbf{u}^{next}$ based on probalistic model

Posterior of the probalistic model by Equation \@ref(eq:post-mean-cov) can quantify the uncertainty over the space of the unknown function, $f$. The question is what is the next $\mathbf{u}^{next}$ to be sampled from the *expensive function*? One could select the next point arbitrarily but that would be wasteful.
...
```{=tex}
\begin{equation}
utility(\mathbf{u};\theta^{\ast},\mathcal{D})=\alpha_{EI}=\int_{y}^{}max(0,\mathbf{J}(u_*)-f)p(\mathbf{J}(\mathbf{u_*})|\mathcal{D}) \,dy
\label{eq:utiint}
\end{equation}
```
...
```{=tex}
\begin{equation}
utility(\mathbf{u};\theta,\mathcal{D})=\alpha_{EI}=\int_{y}^{}max(0,\mathbf{J}(\mathbf{u_*})-\mathbf{J}^+)p(\mathbf{J}(\mathbf{u_*})|\mathcal{D}) \,dy
\label{eq:utiint2}
\end{equation}
```
...
```{=tex}
\begin{equation}
\gamma(\mathbf{u_*})=\frac{\mu(\mathbf{u_*;\theta^{\ast},\mathcal{D}})-\mathbf{J}^{+}}{\sigma(\mathbf{\mathbf{u_*};\theta^{\ast},\mathcal{D}})}
\label{eq:gamma}
\end{equation}
```
...
```{=tex}
\begin{equation}
utility(\mathbf{u_*};\theta,\mathcal{D})=\alpha_{EI}(\mathbf{u_*};\theta,\mathcal{D})=(\mu(\mathbf{u_*};\theta,\mathcal{D})-\mathbf{J}^{+})\Phi(\gamma(\mathbf{u
_*})) + \sigma(\mathbf{u_*};\theta,\mathcal{D})\phi(\gamma(\mathbf{u_*}))
\label{eq:utility}
\end{equation}
```
...
```{=tex}
\begin{equation}
\gamma(\mathbf{u_*})=\frac{\mu(\mathbf{u_*};\theta,\mathcal{D})-\mathbf{J}^+-\epsilon}{\sigma(\mathbf{u_*};\theta,\mathcal{D})}
...
```{=tex}
\begin{equation}
\alpha_{EI}(\mathbf{u_*};\theta,\mathcal{D})=(\mu(\mathbf{u_*};\theta,\mathcal{D})-\mathbf{J}^{+}-\epsilon)\Phi(\gamma(\mathbf{\mathbf{u_*}})) + \sigma(\mathbf{u_*};\theta,\mathcal{D})\phi(\gamma(\mathbf{u_*}))

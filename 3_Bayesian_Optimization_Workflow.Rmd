...
In Equation \@ref(eq:mean-cov), $m(\mathbf{u})$ is a mean function and $\kappa(\mathbf{u},\mathbf{u'})$ is a covariance function (or kernel). $\kappa(\mathbf{u},\mathbf{u'})$ specifies the similarity between two values of a function evaluated on $\mathbf{u}$, and $\mathbf{u'}$ . The GPR is a distribution over function completely defined by its mean and covariance function as:

```{=tex}
\begin{equation}
\mathbf{J}(\mathbf{u}) \sim \mathcal{MN}(m(\mathbf{u}), \kappa(\mathbf{u},\mathbf{u'}))
...
```{=tex}
\begin{equation}
\mathbf{K}_{U,U}=\kappa(\mathbf{u_i,u_j})=\left (
\begin{array}{ccc}
\begin{array}{l}

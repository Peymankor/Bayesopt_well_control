...
Generalized Pattern-search (GPS) methods (Dennis, n.d.; Torczon, 1997) are another types of gradient free techinques has been has been apliied in well control optimization problem. Pattern-search method relies on polling, at any particular iteration a stencil is centered at the current solution. The stencil comprises a set of directions such that at least one is a descent direction. If some of the points in the stencil represent an improvement in the cost function, the stencil is moved to one of these new solutions. GPS and including variante of it (Mesh Adaptive direct search) usually considered as local search methods but due to ability to parallization, it has been a one of the favorite method in well control optimization. (Asadollahi et al., 2014; Echeverría Ciaurri et al., 2010; Foroud and Seifi, 2016; Nwachukwu et al., 2018a). To overcome the locality of the GPS methods, hybrid of the PSO as global method and then using pattern search for local search in iterative (PSO-MADS) way as well proposed in the work of (de Brito and Durlofsky, 2020; Isebor et al., 2013).
...
In the previous section we reviewed commonly used optimization algorithm in well control optimization. However, it should be noted that even with low cost optimization method like Enopt, still performing full RO optimization is computationally prohibitive (we will discuss about detail of this computational burden more in Section 2). To make RO feasible, two general approaches have been introduced. The first set of techniques targets the flow problem. This generally entails approximating the high-fidelity flow simulation model with a lower-fidelity or reduced order model, or with a model based on simplifed flow physics. To get insight how this line of research is evolving , we did a small text mining task.Mining the more than 100000 peer-reviewed papers published in (www.onepetro.org), we count the number of the papers with ``one'' of following keyword in their abstract:
...
The time period of 1995-2020 were considered. As the \ref{fig:onepetroanalysis}
...
\leavevmode\vadjust pre{\hypertarget{ref-dennisa}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-nwachukwu2018a}{}}%

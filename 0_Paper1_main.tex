...
In \eqref{eq:post-mean-cov-single}, we replaced the \(\mathcal{MN}\) with \(\mathcal{N}\) in \eqref{eq:post-mean-cov} as Equation \eqref{eq:post-mean-cov-single} shows the probality of \(\mathbf{J}\)for \emph{one} conrol variable, where in Equation \eqref{eq:post-mean-cov} we have th probality of the \(\mathbf{J}\), over vector of control variable, \(\mathbf{U}\).
...
The utility defined in Equation @(ref:utiint) can be seen as expected value improvemnet posterior of the model regrading to the \emph{true function} at point \(\mathbf{u_\ast}\). However, we do not have access to the \emph{expensive function}, \(f\), therefore we replace the \(f\) with the best available solution found so far, \(\mathbf{J}^+\). The \(\mathbf{J^+}\) mathematically can be defined, then as \eqref{eq:utiint} can be written as \eqref{eq:utiint2} as below:
...
After applying some tedious integration by parts on right side of \eqref{eq:utiint2}, one can express the expected improvement in closed form (Jones et al., 1998). To achieve closed form, first we need some parametrization and define the \(\gamma(\mathbf{u_*})\) as below:
...
Where \(\mathbf{\mu_{u_\ast}}\) and \(\sigma_\mathbf{u_{\ast}}\) can be found from Eqaution \eqref{eq:post-mean-cov-single} and \(\mathbf{J^+}\) has been defined in Equation \eqref{eq:j-plus}. Given the \(\gamma(\mathbf{u_*})\), the right side of Equation \eqref{eq:utiint2} can be written as:
...
Where \(\Phi(.)\) and \(\phi(.)\) are CDF and PDF of standard Gaussian distribution. We need to note that \(\alpha_{EI}(\mathbf{u_*};\theta^*,\mathcal{D})\) is always non-negative number, as it the integral defined in \eqref{eq:utiint2} is truncating the negative side of the function \(\mathbf{J}\) inside the \(max()\) term. The Equation\eqref{eq:utility} does the fine job in many of application of BAyesian Optimization. However, the utility defined in the Equation \eqref{eq:utility} sometimes can be \emph{greedy}. In this contex, greedy utility means that focus more on the ``immediate reward,'' whic is the first part of Equation \eqref{eq:utility}, less on the ``Exploration'' part.Therefore to avoid this greed an dmake the utility function more forward looking, an explorative term is added as \(\epsilon\) and Equation \eqref{eq:gamma} can be re-written as:
...
\leavevmode\vadjust pre{\hypertarget{ref-jones1998}{}}%
Jones, D.R., Schonlau, M., Welch, W.J., 1998. Efficient Global Optimization of Expensive Black-Box Functions. Journal of Global Optimization 13, 455--492. doi:\href{https://doi.org/10.1023/A:1008306431147}{10.1023/A:1008306431147}
